...
class ANFIS:
    ...
    def Train4MRbC(self, Train_index, Val_index, k, epoch = 500, lam=0):
        ...
        for i in trange(epoch):
            ...
            self.MR = self.X[R_index[:2]]
            self.Train(Train_index, epoch=1, lam=lam)
            Err_t = self.Err_Rate(self.prediction(self.X[Val_index]), self.Y[Val_index], "All")

            if Err_t < Err:
                MR_b, Err = self.MR, Err_t
            if Err < 1:
